...
and applies macOS Wi-Fi automatic proxy settings only while a VPN is connected.
...
- macOS `networksetup` integration for Wi-Fi automatic proxy configuration.
...
ドメイン一覧、エラー状態を確認できます。`Add`、`Apply`、`Config`、`Quit`
の操作と、`Open at Login` の切り替えもここから実行できます。
...
VPN 接続中は `networksetup` で Wi-Fi の自動プロキシ URL を
`http://127.0.0.1:<pac_port>/proxy.pac` に設定し、有効化します。停止時または
VPN 切断時は自動プロキシを無効化します。
...
- A `~/.config/crabbyproxy/config.toml` file if you want persistent custom
  domains and ports
- Permission to run macOS `networksetup` commands for Wi-Fi proxy settings
...
- Connect your VPN to let ProxyBar apply the PAC URL to Wi-Fi.
...
- Turn ProxyBar off or quit the app to disable Wi-Fi automatic proxy settings.
